...
class IsoOSBITorsionModel:   
    ...
    def __init__(self, rmbm, tbx, zetabx, ebxd, wrwxb, efxd, f0, g1, bt, g2, a, nt, D, ecc, rmrm, am, niso, eb0xd, tbe0x):
        self.rmbm = rmbm
        self.tbx = tbx
        self.tbe0x = tbe0x
        self.zetabx = zetabx
                ...
                self.zetabx == other.zetabx and self.ebxd == other.ebxd and  self.wrwxb == other.wrwxb and \
                    self.efxd == other.efxd and self.f0 == other.f0 and self.g1 == other.g1 and self.bt == other.bt and \
                        self.g2 == other.g2 and self.a == other.a and self.nt == other.nt and self.D == other.D and self.ecc == other.ecc and \
                            self.rmrm == other.rmrm and self.am == am and self.niso == other.niso and self.eb0xd == other.eb0xd and self.tbe0x == tbe0x
        return False
...
def read_iso_osbi_torsion_var_param(var_param_file, fm, niso):
    ...
    dtype1 = np.dtype([('IJK', 'i4'), ('RMBM', 'd'), ('TBX', 'd'), ('ZETABX','d'), ('EBXD','d'), ('WRWXB', 'd'), ('efxd', 'd'), ('F0', 'd'), ('G1', 'd'), ('BT', 'd'), ('G2', 'd'), ('A', 'd'), ('NT', 'd'), ('D', 'd'), ('ECC', 'd'), ('RMRM', 'd'), ('EB0XD', 'd'), ('TB0XD', 'd')])
    
    ijk, rmbm, tbx, zetabx, ebxd, wrwxb, efxd, f0, g1, bt, g2, a, nt, D, ecc, rmrm, eb0xd, tbe0x = np.loadtxt \
    ...
    for i in range(0, total_param):
        try:
            yield IsoOSBITorsionModel(rmbm[i], tbx[i], zetabx[i], ebxd[i], wrwxb[i], efxd[i], f0[i], g1[i], bt[i], g2[i], a[i], nt[i], D[i], ecc[i], rmrm[i], fm, niso, eb0xd[i], tbe0x[i])
        except:
            yield IsoOSBITorsionModel(rmbm, tbx, zetabx, ebxd, wrwxb, efxd, f0, g1, bt, g2, a, nt, D, ecc, rmrm, fm, niso, eb0xd, tbe0x)
