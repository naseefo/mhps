...
C         OPEN(2,FILE='Northridge_Sylmar.txt')
        ...
        READ(1,*) KNOR,KSN
        READ(1,*) N
	  WRITE(*,*) 'DUDE I HAVE REACHED HERE AT N'
...
c  NASEEF: For the isolator to be linear it has to obey 
c  the following condition in the bilinear formulation
       F0 = G1*CKAB/(TM*9.81)
	  FY = F0 * TM * 9.81 
...
102     CALL INV(V,VI,NST)
	   ...
	   WRITE(*,*)((SK(I,J),J=1,NDOF),I=1,NDOF)
...
C       WRITE(*,1316) K,TIME,FAB,RF
        WRITE(4,1346) TIME, EQ, AA(1),V2(1),D2(NDOF), RF, FAB1
C	1  ,FAB
 1020   CONTINUE
...
C	  WRITE(*,*)ABS(PD(NDOF))*100
	  ...
	  WRITE(5,1356) IJK,ABS(PA(1)),ABS(PD(NDOF))
