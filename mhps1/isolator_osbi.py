...
def fb(J, mr, drb, dyb, dxb, vrb, vyb, vxb, arg, ayg, axg, arb, ayb, axb, p_d0, theta_r_dot2, c_d0, y_b_d2):
    ...
    fbx = 1.0*(fbr1x + fbr2x + fbr3x + fbr4x)

    fby = 1.0*(fbr1y + fbr2y + fbr3y + fbr4y)
...
def mu_val(iso, ub):
    ...
    L1 = (iso.alpha1-iso.alpha0)*(iso.mu0/iso.fos_ud*iso.umax)*abs(ub)
    L2 = (iso.alpha1-iso.alpha0)*(iso.mu0/pow(iso.fos_ud*iso.umax,2))*pow(ub,2)
    if iso.typevf == 1:
...
def simulator_osbi(ref, xg, yg, dt, ndiv, ndt, lxy, ijk, ndof, smx, skx, cdx, smy, sky, cdy, iso, screen_on):
    ...
    for i in range(1, len(xg)):
        ...
        if rolling_state == False:
            ...
            fs2x2, fs2y2 = 1111, 1111
            fs1x2, fs1y2 = 1111, 1111
            ...
            for ii in range (nit):
                ...
                # if t >= 4.3110 and t <=4.3170 and ii == nit-1: 
                #     print("Velocity in resultant : %15.12f m/s | vx2: %15.12f m/s | vy2: %15.12f m/s "%(vrb, vx2[ndof-1, 0], vy2[ndof-1, 0]))
                ...
                if vrb > 1.0e-5:
                    ...
                    ax22 = ax1[ndof-1, 0] + daxb
                    ay22 = ay1[ndof-1, 0] + dayb

                    drb = sqrt(pow(dx2[ndof-1, 0], 2.0) + pow(dy2[ndof-1, 0], 2.0))
                    arb = sqrt(pow(ax22, 2.0) + pow(ay22, 2.0))
                    ...
                    fbx2, fby2 = fb(iso.Jr, iso.Mr, drb, dy2[ndof-1, 0], dx2[ndof-1, 0], vrb, vy2[ndof-1, 0], vx2[ndof-1, 0], arg, yg[i], xg[i], arb, ay22, ax22, p_d02, theta_r_dot22, c_d02, ya2)
            ...
            # if t >= 4.3110 and t <=4.3170:
            #     print("OSBI: Slide at t : %8.4f s | fabx = % 8.4f N | faby = % 8.4f N | ddx: % 8.4f m | ddx: % 8.4f m  "%(t, fabx, faby, ddx[ndof-1], ddy[ndof-1]))
            #     print("OSBI: Slide at t : %8.4f s | fbx = % 8.4f N | fby = % 8.4f N | fs1x: % 8.4f m | fs1y: % 8.4f m  "%(t, fbx, fby, fs1x, fs1y))
            #     print()
            #     print("OSBI: Slide at t : %8.4f s |qx: % 8.4f | fcx: % 8.4f m | fcy: % 8.4f m | ya2: % 8.4f m "%(t, fs2x2, fcx, fcy, ya2))
